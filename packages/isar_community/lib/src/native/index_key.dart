...
Pointer<CIndexKey> buildUpperUnboundedIndexKey() {
  isar_key_create(_keyPtrPtr);
  final keyPtr = _keyPtrPtr.value;
  isar_key_add_long(keyPtr, maxLong);
...
void _addKeyValue(
  ...
  final isarType = type != IndexType.hash
      ? property.type.scalarType
      : property.type;
  switch (isarType) {
    case IsarType.bool:
      isar_key_add_byte(keyPtr, (value as bool?).byteValue);
      break;
    case IsarType.byte:
      isar_key_add_byte(keyPtr, (value ?? 0) as int);
      break;
    case IsarType.int:
      isar_key_add_int(keyPtr, (value as int?) ?? nullInt);
      break;
    case IsarType.float:
      isar_key_add_float(keyPtr, (value as double?) ?? nullFloat);
      break;
    case IsarType.long:
      isar_key_add_long(keyPtr, (value as int?) ?? nullLong);
      break;
    case IsarType.double:
      isar_key_add_double(keyPtr, (value as double?) ?? nullDouble);
      break;
    case IsarType.dateTime:
      isar_key_add_long(keyPtr, (value as DateTime?).longValue);
      break;
    case IsarType.string:
      final strPtr = _strToNative(value as String?);
      if (type == IndexType.value) {
        isar_key_add_string(keyPtr, strPtr, caseSensitive);
      } else {
        isar_key_add_string_hash(keyPtr, strPtr, caseSensitive);
      }
      _freeStr(strPtr);
      break;
    case IsarType.boolList:
      if (value == null) {
        isar_key_add_byte_list_hash(keyPtr, nullptr, 0);
      } else {
        value as List<bool?>;
        ...
        isar_key_add_byte_list_hash(keyPtr, boolListPtr, value.length);
    ...
    case IsarType.byteList:
      if (value == null) {
        ...
        isar_key_add_byte_list_hash(keyPtr, bytesPtr, value.length);
        malloc.free(bytesPtr);
      }
      break;
    case IsarType.intList:
      if (value == null) {
        isar_key_add_int_list_hash(keyPtr, nullptr, 0);
      } else {
        value as List<int?>;
        ...
        isar_key_add_int_list_hash(keyPtr, intListPtr, value.length);
        malloc.free(intListPtr);
      }
      break;
    case IsarType.longList:
      if (value == null) {
        isar_key_add_long_list_hash(keyPtr, nullptr, 0);
      } else {
        value as List<int?>;
        ...
        isar_key_add_long_list_hash(keyPtr, longListPtr, value.length);
    ...
    case IsarType.stringList:
      if (value == null) {
        isar_key_add_string_list_hash(keyPtr, nullptr, 0, false);
      } else {
